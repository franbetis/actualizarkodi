...
def historico(item):
    logger.info("pelisalacarta.channels.motogpennegro historico")
    itemlist = []
    data = scrapertools.cachePage(item.url)
...
def play_flash(item):
    logger.info("pelisalacarta.channels.motogpennegro play_flash")
    data = scrapertools.cachePage(item.url)
    url = decode_adfly(data)
    host = urlparse.urljoin(url, '/')
    url = "catcher=%s&url=%s&referer=%s" % ("streams", url, host)
    import xbmc
