...
def mainlist(item):
    logger.info("pelisalacarta.channels.boxingclub mainlist")
    itemlist = []
...
def novedades(item):
    logger.info("pelisalacarta.channels.boxingclub novedades")
    itemlist = []
...
def search(item, texto):
    logger.info("pelisalacarta.channels.boxingclub search")
...
def ufc(item):
    logger.info("pelisalacarta.channels.boxingclub ufc")
    itemlist = []
    data = scrapertools.cachePage(item.url)
    data = re.sub(r"\n|\r|\t", '', data)
...
def wwe(item):
    logger.info("pelisalacarta.channels.boxingclub wwe")
    itemlist = []
    data = scrapertools.cachePage(item.url)
    data = re.sub(r"\n|\r|\t", '', data)
...
def wwe_entradas(item):
    logger.info("pelisalacarta.channels.boxingclub wwe_entradas")
    itemlist = []
    data = scrapertools.cachePage(item.url)
    data = re.sub(r"\n|\r|\t", '', data)
