...
def mainlist(item):
    logger.info("pelisalacarta.channels.fullmatchtv mainlist")
    itemlist = []
...
def search(item,texto):
    logger.info("pelisalacarta.channels.fullmatchtv search")
    itemlist = []
    item.url = "http://fullmatchtv.com/?s=" + texto
    return busqueda(item)

def novedades(item):
    logger.info("pelisalacarta.channels.fullmatchtv novedades")
    itemlist = []
    if item.url.startswith("action"):
        data = scrapertools.cachePage("http://fullmatchtv.com/wp-admin/admin-ajax.php?td_theme_name=Newspaper&v=6.6.2", post=item.url)
    ...
    for scrapedurl, scrapedtitle, scrapedthumbnail, mes, dia, year in matches:
        ...
        itemlist.append(Item(channel=__channel__, title=bbcode_kodi2html(scrapedtitle), url=scrapedurl, action="findvideos", thumbnail=scrapedthumbnail, fanart=item.fanart, folder=True))
    ...
    if int(current_page) < int(max_page):
        ...
        itemlist.append(Item(channel=__channel__, title=">> Siguiente", url=item.url, action="novedades", extra=next_page, thumbnail=item.thumbnail, fanart=item.fanart, folder=True))

    return itemlist

def categorias(item):
    logger.info("pelisalacarta.channels.fullmatchtv categorias")
    itemlist = []
    ...
    itemlist.append(Item(channel=__channel__, title="Baloncesto", url=post % "114", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))
    itemlist.append(Item(channel=__channel__, title="Fútbol", url=post % "119", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))
    itemlist.append(Item(channel=__channel__, title="Fútbol Americano", url=post % "115", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))
    itemlist.append(Item(channel=__channel__, title="Béisbol (MLB)", url=post % "116", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))
    itemlist.append(Item(channel=__channel__, title="Motor", url=post % "1110", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))
    itemlist.append(Item(channel=__channel__, title="NHL", url=post % "117", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))
    itemlist.append(Item(channel=__channel__, title="Rugby", url=post % "120", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))
    itemlist.append(Item(channel=__channel__, title="Otros Deportes", url=post % "1150", action="novedades", thumbnail=item.thumbnail, extra="1", fanart=item.fanart, folder=True))

    return itemlist

def busqueda(item):
    logger.info("pelisalacarta.channels.fullmatchtv busqueda")
    itemlist = []
    data = scrapertools.cachePage(item.url)
    data = scrapertools.decodeHtmlentities(data)
    ...
    if len (next_page) > 0:
        itemlist.append(Item(channel=__channel__, title=">> Siguiente", url=next_page, action="busqueda", thumbnail=item.thumbnail, fanart=item.fanart, folder=True))
